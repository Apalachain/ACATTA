...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.dropout = nn.Dropout2d(p=0.5) ## 默认是0.3
...
class STCN(nn.Module):
    ...
    def encode_key(self, frame,ema_model=None): 
        ...
        if ema_model==None:
            ...
            #f16, f8, f4 = self.key_encoder(frame.flatten(start_dim=0, end_dim=1))
            f16, f8, f4 = ema_model.key_encoder(frame.flatten(start_dim=0, end_dim=1))
            k16 = self.key_proj(f16)
            f16_thin = self.key_comp(f16)
    ...
    def segment(self, qk16, qv16, qf8, qf4, mk16, mv16, selector=None):
        ...
        if self.single_object: #false?
            ...
            logits = torch.cat([
                ...
                for i in range(mv16.shape[1]) ##应该是object数量
            ], 1)
