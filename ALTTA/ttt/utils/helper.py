...
def video_inference_not_youtube(data, prop_model, output_dir, args):
    with torch.cuda.amp.autocast(enabled=args.amp):
        ...
        if output_dir is not None:
            # Save the results
            video_output_dir = path.join(output_dir, args.res_path, name)
            os.makedirs(video_output_dir, exist_ok=True)
            for f in range(out_masks.shape[0]):
                img_E = Image.fromarray(out_masks[f])
                
                #img_E.putpalette(args.palette)
                #img_E.save(os.path.join(video_output_dir, 'frame_{}.png'.format(f)))
                scores.append(metric.binary.dc(out_masks[f],msk[0,f,0].cpu().numpy().astype(np.uint8)))
                #print("{},frame{},score is {}".format(info['name'],f,scores[f]))
            #     sample_acc_list.append(('frame_{}.png'.format(f),round(scores[f],4)))
            # # 将列表转换为DataFrame
            # df = pd.DataFrame(sample_acc_list, columns=['Sample ID', 'Accuracy'])
            # excel_path='{}/acc.xlsx'.format(video_output_dir)
            # with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            #     df.to_excel(writer, index=False, float_format='%.2f')
        print(f"{name} score:{sum(scores)/len(scores)}")
        return processor.prob, stats
...
def get_frame_dirs(args):
    if args.corrupted_image_dir is None:
        if args.dataset_name == "youtube":
            ...
            frame_dir = path.join(args.dataset_dir,'JPEGImages', '480p')
            all_frames_dir = None
        elif args.dataset_name == "mose":
            frame_dir = path.join(args.dataset_dir, args.split, 'JPEGImages')
            all_frames_dir = None
        elif args.dataset_name == "SurgicalVideo":
...
def get_palette(args):
    if args.dataset_name == "youtube":
        ...
        palette_filename = args.dataset_dir + f'/Annotations/480p/{palette_video_name}/00000.png'
    elif args.dataset_name == "mose":
        ...
        palette_video_name="video2"
        palette_filename=args.dataset_dir+ f'/{palette_video_name}/frame_0.npz.npy'
    #palette = Image.open(path.expanduser(palette_filename)).getpalette()
    palette = np.load(palette_filename,allow_pickle=True)
    palette=palette.item()
    palette=palette['image']
    palette_image = Image.fromarray(palette)
    palette_image=palette_image.getpalette()
    return palette_image #无调色板
...
def get_test_dataset(args):
    ...
    if args.dataset_name == "youtube":
        ...
        if args.split == 'val':
            ...
            test_dataset = DAVISTestDataset(args.dataset_dir,imset='2017/val.txt',
                                            corrupt_dir=args.corrupted_image_dir)
        ...
        test_dataset = SurgicalTestDataset(args.dataset_dir,corrupt_dir=args.corrupted_image_dir)
    return test_dataset
